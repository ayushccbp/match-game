...
import './index.css'
import GameListTab from '../ImageCategoryTab'
...
class MatchGame extends Component {
  ...
  onClickThumbnailImage = thumbnailId => {
    const {matchImage, score} = this.state
    const {id} = matchImage
    if (id === thumbnailId) {
      const updateScore = score + 1
      const index = Math.floor(Math.random() * imagesList.length - 1)
      const randomImage = imagesList[index]
      this.setState({matchImage: randomImage, score: updateScore})
    } else {
  ...
  gameOver = () => {
    ...
    return (
      <div className="game-over-container">
        ...
        />
        <h1 className="game-result-score">YOUR SCORE</h1>
        <h1 className="game-result-score">{score}</h1>
        <button
  ...
  gameRunning = () => {
    const {matchImage, selectedTabId, time} = this.state
    ...
    return (
      <>
        <div className="game-image-container">
          <img className="game-image" alt="match" src={matchImage.imageUrl} />
        </div>
        <ul className="game-image-tab-list-container">
        ...
        <ul className="game-thumbnail-container">
  ...
  render() {
    const {time, score, isGameActive} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        <div className="game-bg-container">
          {isGameActive ? this.gameRunning() : this.gameOver()}
        </div>
      </div>
    )
  }
